...
The preload bridge exposes a typed renderer API over Electron IPC while preserving context isolation. It is the only renderer-facing bridge to setup, model, chat, debug log, tools, workspace, directory picker, file streaming, raw chunk, and audio transcription operations.
...
- Expose setup, model, chat, log, tools, workspace, dialog, streaming, and transcription methods on window.api.
