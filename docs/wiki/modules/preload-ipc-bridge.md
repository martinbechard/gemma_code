...
The preload bridge exposes a typed renderer API while preserving context isolation. It is the renderer-facing bridge to setup, configured model listing, model switching, remote credential status and saving, chat, debug log, tools, workspace, directory picker, clipboard writes, file streaming, raw chunk, and an audio transcription IPC method.
...
- Expose setup, model list, model switching, remote credential, chat, log, tools, workspace, dialog, clipboard, streaming, and audio transcription methods on window.api.
...
- getRemoteCredentialStatus and saveRemoteCredential expose cloud provider key management without returning secret values to the renderer.
...
- Preload parity is indirectly covered by renderer and main tests. Setup error copy and remote credential UI are covered by [Setup component tests](../../../tests/renderer/components/Setup.test.ts).
